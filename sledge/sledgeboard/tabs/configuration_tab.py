...
from sledge.sledgeboard.base.base_tab import BaseTab
...
from sledge.sledgeboard.style import configuration_tab_style

logger = logging.getLogger(__name__)


class ConfigurationTab:
    """Configuration tab for sledgeboard."""

    def __init__(self, doc: Document, experiment_file_data: ExperimentFileData, tabs: List[BaseTab]):
        """
        Configuration tab about configuring sledgeboard.
        ...
        self._file_path_input = FileInput(
            accept=SledgeBoardFile.extension(),
    ...
    def add_sledgeboard_file_to_experiments(self, sledgeboard_file: SledgeBoardFile) -> None:
        """
        Add sledgeboard files to experiments.
        :param sledgeboard_file: Added sledgeboard file.
        """
        sledgeboard_file.current_path = Path(sledgeboard_file.metric_main_path)
        if sledgeboard_file not in self.experiment_file_data.file_paths:
            self.experiment_file_data.update_data(file_paths=[sledgeboard_file])
    ...
    def _add_experiment_file(self, attr: str, old: bytes, new: bytes) -> None:
        ...
        try:
            ...
            sledgeboard_file = SledgeBoardFile.deserialize(data=data)
            self.add_sledgeboard_file_to_experiments(sledgeboard_file=sledgeboard_file)
            file_stream.close()
