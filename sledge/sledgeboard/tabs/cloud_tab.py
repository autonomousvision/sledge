...
from sledge.sledgeboard.tabs.config.cloud_tab_config import (
...
from sledge.sledgeboard.tabs.configuration_tab import ConfigurationTab
from sledge.sledgeboard.tabs.js_code.cloud_tab_js_code import (
...
from sledge.sledgeboard.utils.sledgeboard_cloud_utils import (
    S3FileResultMessage,
    S3SledgeBoardFileResultMessage,
    check_s3_sledgeboard_files,
    download_s3_file,
    download_s3_path,
...
class CloudTab:
    ...
    def _s3_download_prefixes(self) -> None:
        ...
        try:
            ...
            s3_sledgeboard_file_result = check_s3_sledgeboard_files(
                s3_result_file_contents.file_contents, s3_client=self._s3_client, s3_path=selected_s3_path
            )
            if not s3_sledgeboard_file_result.s3_connection_status.success:
                raise Boto3Error(s3_sledgeboard_file_result.s3_connection_status.return_message)
            ...
            self._update_s3_sledgeboard_file_main_path(
                s3_sledgeboard_file_result=s3_sledgeboard_file_result, selected_prefix=selected_s3_prefix
            )
    ...
    def _update_s3_sledgeboard_file_main_path(
        self, s3_sledgeboard_file_result: S3SledgeBoardFileResultMessage, selected_prefix: str
    ) -> None:
        """
        Update sledgeboard file simulation and metric main path.
        :param s3_sledgeboard_file_result: S3 sledgeboard file result.
        :param selected_prefix: Selected prefix on s3.
        """
        sledgeboard_file = s3_sledgeboard_file_result.sledgeboard_file
        sledgeboard_filename = s3_sledgeboard_file_result.sledgeboard_filename
        if not sledgeboard_file or not sledgeboard_filename or not self._nuplan_exp_root:
            return
        main_path = Path(self._nuplan_exp_root) / selected_prefix
        sledgeboard_file.simulation_main_path = str(main_path)
        sledgeboard_file.metric_main_path = str(main_path)
        metric_path = main_path / sledgeboard_file.metric_folder
        if not metric_path.exists():
            metric_path.mkdir(parents=True, exist_ok=True)

        simulation_path = main_path / sledgeboard_file.simulation_folder
        if not simulation_path.exists():
            simulation_path.mkdir(parents=True, exist_ok=True)

        aggregator_metric_path = main_path / sledgeboard_file.aggregator_metric_folder
        if not aggregator_metric_path.exists():
            aggregator_metric_path.mkdir(parents=True, exist_ok=True)

        # Replace with new local path
        save_path = main_path / sledgeboard_filename
        sledgeboard_file.save_sledgeboard_file(save_path)
        logger.info("Updated nubBard main path in {} to {}".format(save_path, main_path))
        self._configuration_tab.add_sledgeboard_file_to_experiments(sledgeboard_file=s3_sledgeboard_file_result.sledgeboard_file)
