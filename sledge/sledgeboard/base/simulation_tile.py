...
from sledge.sledgeboard.base.data_class import SimulationScenarioKey
from sledge.sledgeboard.base.experiment_file_data import ExperimentFileData
from sledge.sledgeboard.base.plot_data import MapPoint, SimulationData, SimulationFigure
from sledge.sledgeboard.style import simulation_map_layer_color, simulation_tile_style
from sledge.simulation.scenarios.sledge_scenario.sledge_scenario import SledgeScenario
from sledge.sledgeboard.tabs.config.scenario_tab_config import (
...
class SimulationTile:
    ...
    def _create_frame_control_button(
        self,
        button_config: ScenarioTabFrameButtonConfig,
        click_callback: EventCallback,
        figure_index: int,
    ) -> Button:
        """
    ...
    def _create_initial_figure(
        ...
        experiment_path = Path(
            self._experiment_file_data.file_paths[selected_scenario_key.sledgeboard_file_index].metric_main_path
        )
        ...
        # simulation_figure.outline_line_width = 7

        # simulation_figure.xgrid[0].level = 'underlay'
        # simulation_figure.ygrid[0].level = 'underlay'
        ...
        simulation_figure_data = SimulationFigure(
            ...
            file_path_index=selected_scenario_key.sledgeboard_file_index,
    ...
    def _video_button_next_tick(self, figure_index: int) -> None:
        ...
        sledgeboard_file_index = scenario_key.sledgeboard_file_index
        video_path = (
            Path(self._experiment_file_data.file_paths[sledgeboard_file_index].simulation_main_path)
            / "video_screenshot"
        )
        ...
        try:
            if len(selected_simulation_figure.ego_state_plot.data_sources):
                ...
                images_save_path = video_path / simulation_figure.scenario.token
                ...
                for frame_index in tqdm(range(length), desc="Rendering video"):
                    ...
                    image.save(images_save_path / f"frame_{frame_index}.png")
            ...
            try:
                ...
                logger.info("Couldn't save video on failure")
    ...
    def _load_map_data(self, main_figure: SimulationFigure) -> None:
        ...
        if isinstance(main_figure.scenario, SledgeScenario):
            map_api = main_figure.scenario.map_api
        else:
        ...
        start_pose, end_pose = (
            main_figure.simulation_history.data[0].ego_state.center,
            main_figure.simulation_history.data[-1].ego_state.center,
        )
        distance = ((start_pose.array - end_pose.array) ** 2).sum(-1) ** 0.5

        self._nearest_vector_map = map_api.get_proximal_map_objects(center, distance + 64, layer_names)
        ...
        if isinstance(main_figure.scenario, SledgeScenario):
            map_layer = SemanticMapLayer.LANE
        else:
            map_layer = SemanticMapLayer.LANE_CONNECTOR

        main_figure.lane_connectors = {
            lane_connector.id: lane_connector for lane_connector in self._nearest_vector_map[map_layer]
        }
    ...
    def _render_map_polygon_layers(self, main_figure: SimulationFigure) -> None:
        ...
        polygon_layer_names = [
            ...
            (
                SemanticMapLayer.INTERSECTION,
                simulation_map_layer_color[SemanticMapLayer.INTERSECTION],
            ),
            ...
            (
                SemanticMapLayer.CARPARK_AREA,
                simulation_map_layer_color[SemanticMapLayer.CARPARK_AREA],
            ),
        ]
