...
@dataclass(frozen=True)
class S3SledgeBoardFileResultMessage:
    """Data class to save aws return messages and sledgeboard file."""

    s3_connection_status: S3ConnectionStatus
    sledgeboard_filename: Optional[str] = None
    sledgeboard_file: Optional[SledgeBoardFile] = None


def check_s3_sledgeboard_files(
    s3_file_contents: Dict[str, S3FileContent], s3_path: str, s3_client: boto3.client
) -> S3SledgeBoardFileResultMessage:
    """
    Return True in the message if there is a sledgeboard file and can load into SledgeBoard.
    ...
    :return S3SledgeBoardFileResultMessage to indicate if there is available sledgeboard file in the s3 prefix.
    """
    success = False
    return_message = "No available sledgeboard files in the prefix"
    sledgeboard_file = None
    sledgeboard_filename = None
    if not s3_path.endswith("/"):
        s3_path = s3_path + "/"
    url = parse.urlparse(s3_path)
    for file_name, file_content in s3_file_contents.items():
        if file_name.endswith(SledgeBoardFile.extension()):
            try:
                sledgeboard_object = s3_client.get_object(Bucket=url.netloc, Key=file_name)
                file_stream = io.BytesIO(sledgeboard_object["Body"].read())
                sledgeboard_data = pickle.load(file_stream)
                sledgeboard_file = SledgeBoardFile.deserialize(sledgeboard_data)
                file_stream.close()
                sledgeboard_filename = Path(file_name).name
                return_message = f"Found available sledgeboard file: {sledgeboard_filename}"
                success = True
                break
    ...
    return S3SledgeBoardFileResultMessage(
        s3_connection_status=S3ConnectionStatus(success=success, return_message=return_message),
        sledgeboard_filename=sledgeboard_filename,
        sledgeboard_file=sledgeboard_file,
    )
